...
export default function ProfitTrendChart() {
  const { data: profitData = [], isLoading, error } = useQuery({
    queryKey: ["profit-trend-daily"],
    queryFn: () =>
      fetch(`${BASE_URL}/dashboard/profit_trend_daily`).then((res) => {
        if (!res.ok) {
          throw new Error("Failed to fetch daily profit trend");
        }
        return res.json();
      }),
  });
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <TrendingUp className="w-5 h-5 text-purple-600" /> Daily Profit Trend
          </CardTitle>
        </CardHeader>
    ...
    return (
      <Card>
        ...
        <CardContent className="flex items-center justify-center h-64 text-red-600">
          <p>Error loading profit data: {error.message}</p>
        </CardContent>
      </Card>
    );
  }
