...
# Route 1: GET all supply requests (now with clerk_id filtering)
# Added strict_slashes=False to prevent 308 redirects on preflight OPTIONS requests.
# Also explicitly included 'OPTIONS' method for clarity and compatibility.
@supply_bp.route('/', methods=['GET', 'OPTIONS'], strict_slashes=False) 
@mock_jwt_required
def get_all_supply_requests():
    try:
        ...
        clerk_id_from_param = request.args.get('clerk_id')

        query = SupplyRequest.query

        if clerk_id_from_param:
            try:
                query = query.filter_by(clerk_id=int(clerk_id_from_param))
            except (ValueError, TypeError):
                return jsonify({"error": "Invalid clerk_id"}), 400
...
def create_supply_request():
    try:
        ...
        # CORRECTED: Get clerk_id from the JSON payload instead of the mock function
        clerk_id = data.get("clerk_id")

        if not product_id or not store_id or not requested_quantity or not clerk_id:
            return jsonify({"error": "Missing required fields: product_id, store_id, requested_quantity, or clerk_id"}), 400
...
def respond_to_supply_request(request_id):
    ...
    admin_id = mock_get_jwt_identity()
    ...
    # Use the safe serialization logic to prevent AttributeError
    status_str = req.status.value if isinstance(req.status, SupplyRequestStatus) else req.status

    return jsonify({
        "message": f"Request {status_str}.",
        "request": {
            "id": req.id,
            "status": status_str,
